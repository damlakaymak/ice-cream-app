...
isChecked ? setBasket([...basket,item]) 
: setBasket(basket.filter((i)=> i.name !== item.name))
  ...
  return (
    <div>
      ...
      <h3>Soslar Ücreti  <span data-testid="total" className='text-succes'>{basket.length * 3}</span>₺</h3>
...
<div className='top-card col'>
  ...
  <img src= {item.imagePath} height={100}/>
   ...
   <input onChange={(e)=> handleChange(e.target.checked,item)} className='d-none' id={item.name} type='checkbox'/>
</div>
